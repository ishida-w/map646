...
/*
      if(ctime.hour() != 0){
         ...
         jyear = json_object_object_get(jobj, ctime.s_year().c_str());
         ...
         jmonth = json_object_object_get(jyear, ctime.s_month().c_str());
         ...
         jday = json_object_object_get(jmonth, ctime.s_day().c_str());
         ...
         jhour = json_object_object_get(jday, ctime.s_hour().c_str());
         ...
         if(ctime.day() != 1){
            ...
            json_object *jyesterday = NULL;
            int i = -1;
            while(jyesterday == NULL){
               if(i < -30)
                  errx(EXIT_FAILURE, "error while lastday merge. couldn't find unmerged stat data");
               jyesterday = json_object_object_get(jmonth, ctime.s_day(i).c_str());
               i--;
            }

            if(merge(jyesterday) < 0){
               errx(EXIT_FAILURE, "error while merge");
            }
            ...
            if(ctime.month() != 1){
               ...
               json_object *jlastmonth = NULL;
               int i = -1;
               while(jlastmonth == NULL){
                  if(i < -12)
                     errx(EXIT_FAILURE, "error while last month merge. couldn't find unmerged stat data");
                  json_object_object_get(jyear, ctime.s_month(i).c_str());
                  i--;
               }

               if(merge(jlastmonth) < 0){
                  errx(EXIT_FAILURE, "error while merge");
               }
               ...
               json_object *jlastyear = NULL;
               int i = -1;
               while(jlastyear == NULL){
                  if(i < -5)
                     errx(EXIT_FAILURE, "error while last year merge. couldn't find unmerged stat data");
                  json_object_object_get(jobj, ctime.s_year(i).c_str());
                  i--;
               }

               if(merge(jlastyear) < 0){
                  errx(EXIT_FAILURE, "error while merge");
               }
